...
class IDEAndroidLayoutXmlFileManager(val module: Module) : AndroidLayoutXmlFileManager(module.project) {
        ...
        if (androidModule == null) {
            ...
            if (_moduleData == null) {
                _moduleData = cachedValue(project) {
                    com.intellij.psi.util.CachedValueProvider.Result.create(super.getModuleData(), getPsiTreeChangePreprocessor())
                }
            }
        }
    ...
    private fun getPsiTreeChangePreprocessor(): PsiTreeChangePreprocessor {
        return project.getExtensions(com.intellij.psi.impl.PsiTreeChangePreprocessor.EP_NAME).first { it is AndroidPsiTreeChangePreprocessor }
    }
